...
const SignaturesPage = () => {
  ...
  const signedDocuments = documents.filter(
    (doc) =>
      doc.status === "signed" &&
      doc.signedBy &&
      doc.signedBy.includes(user?.name || "")
  );

  // Get documents sent for signature
  const sentForSignature = documents.filter(
    (doc) => doc.ownerId === user?.id && doc.status === "pending_signature"
  );
          ...
          <div className="flex items-center text-green-600 dark:text-green-400 mb-4">
            ...
            <span>
              ...
              {format(
                new Date(
                  Date.now() -
                    1000 * 60 * 60 * 24 * Math.floor(Math.random() * 7)
                ),
                "MMMM d, yyyy"
              )}
            </span>
      ...
      <Card key={document.id}>
        ...
        <CardContent>
          <div className="flex items-center text-muted-foreground mb-4">
            <Clock className="h-4 w-4 mr-1" />
            <span>
              Sent {format(new Date(document.uploadDate), "MMMM d, yyyy")}
            </span>
          </div>
              ...
              <Card>
                <CardContent className="flex flex-col items-center justify-center p-6 text-center">
                  ...
                  <p className="text-muted-foreground text-sm">
                    When someone requests your signature on a document, it will
                    appear here.
                  </p>
                </CardContent>
              </Card>
            ...
            {sentForSignature.length > 0 ? (
              ...
              <Card>
                <CardContent className="flex flex-col items-center justify-center p-6 text-center">
                  ...
                  <p className="text-muted-foreground text-sm">
                    When you send a document for signature, you can track its
                    status here.
                  </p>
            ...
            {signedDocuments.length > 0 ? (
              ...
              <Card>
                <CardContent className="flex flex-col items-center justify-center p-6 text-center">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Documents that you've signed will appear here for your
                    reference.
                  </p>
                </CardContent>
              </Card>
